...
@Component({
  ...
  styleUrls: ['./list-asignatura.component.scss'],
  })
export class ListAsignaturaComponent implements OnInit {
  ...
  loadData(): void {
    this.ubicacionesService.get('asignatura/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.ubicacionesService.delete('asignatura/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'Asignatura deleted');
            }
         });
      }
    });
